/**
 * Polymer-based WebComponent implementation.
 *
 * @remarks
 * Available for backward compatibility.
 * Will be deprecated in a later version.
 * New components should extend the Lit-based WebComponent instead.
 *
 * @internal
 * DO NOT IMPORT FROM THIS FILE DIRECTLY!
 * Use: import * as Polymer from "polymer" and then Polymer.WebComponent instead
 */
...
Settings.setLegacyUndefined(true);
Settings.setOrderedComputed(true);
Settings.setPassiveTouchGestures(true);
Settings.setRemoveNestedTemplates(true);
Settings.setSuppressTemplateNotifications(true);
...
export class WebComponent extends GestureEventListeners(PolymerElement) {
    ...
    private static _updateTemplateProperty(element: CustomElementConstructor, elementName: string) {
        ...
        Object.defineProperty(element, "template", {
            get: () => {
                const template = <HTMLTemplateElement>DomModule.import(`${elementName}-template-module`, "template") || baseTemplate;
                ...
                const userStyleModule = <HTMLTemplateElement>DomModule.import(`${elementName}-style-module`);
                ...
                icons.forEach(icon => {
                    ...
                    IconRegister.add(html`${iconTemplate}`);
    ...
    private static readonly REGISTRATION_PROCESSED = Symbol("PolymerWebComponent.registrationProcessed");
