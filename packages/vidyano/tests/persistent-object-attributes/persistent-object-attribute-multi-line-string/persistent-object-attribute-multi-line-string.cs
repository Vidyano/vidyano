    ...
    .WithModel(builder =>
        {
            ...
            var readOnlyAttr = po.GetOrCreateAttribute(nameof(Mock_Attribute.MultiLineStringReadOnly));
...
public class Mock_Attribute
{
    ...
    public string? MultiLineString { get; set; } = "Line 1\nLine 2\nLine 3";

    public string? MultiLineStringMaxLength { get; set; } = "Test";

    public string? MultiLineStringReadOnly { get; set; } = "Read Only Line 1\nRead Only Line 2";
}
