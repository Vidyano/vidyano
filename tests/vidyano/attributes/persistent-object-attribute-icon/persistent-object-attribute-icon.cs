    ...
    .WithModel(builder =>
        {
            ...
            var iconReadOnly = po.GetOrCreateAttribute(nameof(Mock_Attribute.IconReadOnly));
            iconReadOnly.IsReadOnly = true;

            var iconRequired = po.GetOrCreateAttribute(nameof(Mock_Attribute.IconRequired));
            iconRequired.AddRule("Required");
        })
    );
...
public class Mock_Attribute
{
    ...
    public string? Icon { get; set; } = "Action_Edit";

    [DataType(DataTypes.Icon)]
    public string? IconReadOnly { get; set; } = "Action_New";

    [DataType(DataTypes.Icon)]
    public string? IconRequired { get; set; } = "";

    [DataType(DataTypes.Icon)]
    public string? IconEmpty { get; set; } = null;
}
