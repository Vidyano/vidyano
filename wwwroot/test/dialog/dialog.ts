...
QUnit.module("Dialog tests", hooks => {
    ...
    QUnit.test("Message dialog", async assert => {
        ...
        assert.equal((await waitFor(() => app.shadowRoot.querySelector("vi-message-dialog")?.shadowRoot?.querySelector("dialog > main > pre"))).innerHTML, "Test");
    ...
    QUnit.test("Custom dialog", async assert => {
        ...
        const dialogCore = await waitFor(() => <any>app.shadowRoot.querySelector("vi-my-dialog").shadowRoot.querySelector("dialog") as IronOverlayBehavior);
        assert.ok(dialogCore.opened, "Dialog is opened");

        assert.equal(app.shadowRoot.querySelector("vi-my-dialog").shadowRoot.querySelector("dialog > main > div").innerHTML, "Some content", "Dialog is showing content");

        (app.shadowRoot.querySelector("vi-my-dialog").shadowRoot.querySelector("dialog > footer > vi-button") as Button).click();
    });
    ...
    QUnit.test("Wizard dialog", async assert => {
        ...
        await waitFor(() => app.shadowRoot.querySelector("vi-persistent-object-wizard-dialog")?.shadowRoot.querySelector("dialog > header > h4").innerHTML === "Step 1");
        ...
        (app.shadowRoot.querySelector("vi-persistent-object-wizard-dialog").shadowRoot.querySelector("dialog > footer > div:nth-child(2) > vi-button:nth-child(1)") as Button).click();
        await waitFor(() => app.shadowRoot.querySelector("vi-persistent-object-wizard-dialog").shadowRoot.querySelector("dialog > header > h4").innerHTML === "Step 2");
        ...
        (app.shadowRoot.querySelector("vi-persistent-object-wizard-dialog").shadowRoot.querySelector("dialog > footer > div:nth-child(2) > vi-button:nth-child(2)") as Button).click();
    });
});