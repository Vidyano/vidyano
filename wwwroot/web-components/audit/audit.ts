...
export class Audit extends WebComponent {
    ...
    private async _itemsChanged(items: Vidyano.QueryResultItem[], persistentObject: AuditPersistentObject) {
        ...
        for (let item of newItems) {
            ...
            const logEntry: ILogEntry = {
                ...
                inData: null,
                outData: null,
                href: this.computePath(`${(this.app as App).programUnit.name}/PersistentObject.${item.query.persistentObject.id}/${item.id}`),
    ...
    private _expand(e: Polymer.Gestures.TapEvent) {
